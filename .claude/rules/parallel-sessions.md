...
Shared across sessions; `auto_index` on (no manual `index_repository` unless provably unindexed). **Same machine:** one live in-repo store (concurrent reads, serialised writes) — a second session sees the first's graph + knowledge instantly. **Across machines:** git-paced via the committed conflict-free event log `.lodestar/knowledge/events/**` (set-union; `eol=lf`; `merge.ours.driver` from `setup.sh` 8a; `*.db`/`graph.db.zst` gitignored). Capture verified facts (`knowledge_put`→commit `.lodestar/knowledge/`→push) so peers cite via `knowledge_get`, not re-derivation. `invariant:pure` auto-activates (Stage-1 gate); subjective claims promote from `draft` only via the Stage-2 judge (`LODESTAR_JUDGE_PROVIDER=claude-subagent`, settings.json env; reviewer family must differ from author `claude-code`). Anti-drift: `node_content_hash` anchors → auto `stale`/`contradicted` on change. Full model (both planes, capture discipline, `peer_digest` alignment, lifecycle dashboard): `wiki/playbooks/architecture/orchestration.md §33.1`.
