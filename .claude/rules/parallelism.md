...
**Pod ceiling — MODEL-TIERED, scaled to machine throughput (2026-06-20; supersedes the flat-3):** when write-sets are PROVEN pairwise-disjoint, scale to the Workflow runtime cap `min(16, cores-2)` (≈8–10 concurrent on a 12-core box), MODEL-TIERED: **≤3 concurrent HEAVY large-context opus/fable agents (the empirical 2026-05-18 harness-rejection limit, `feedback_api_load_concurrent_agents` — still the technical cap; 2-of-3 returns got rejected at >3 large-opus), with the remaining slots filled by sonnet/haiku lanes** (lighter models don't hit that limit). The **model-aware** SubagentStart hook (`.claude/hooks/subagent-start-bound-budget.sh`) enforces it: `DESKMODAL_MAX_CONCURRENT_AGENTS_PER_SESSION` (8) total + `DESKMODAL_MAX_CONCURRENT_OPUS_PER_SESSION` (3) heavy; **unknown-model degrades to the safe 3-cap (no regression).** This is **NOT cost-control — cost is not a constraint (ultracode); the cap is purely the technical opus-concurrency limit + core throughput.** To USE the throughput, **decompose into MORE small disjoint packets to fill the lanes (`§4.3`); never under-fill the machine at a flat 3, and never collapse work into one long monolithic job that can fail mid-flight (`feedback_quota_wall_workflow_durability`).** The architectural disjoint-task bound is still `min(16, count_of_disjoint_tasks)`.
