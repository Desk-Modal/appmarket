...
| **[`architecture.md`](architecture.md)** | §16 (service & I/O discipline — non-blocking, service-owns-data) · §17 (plugin invariants — SDKs + Tauri-native decorations) · §19 (OptiScript-everywhere) · §20 (unified config + architecture docs) · §21 (spec hygiene — atomic spec updates per wave) · §23 (FDC3 copilot uniformity pattern) · §24 (file-size + decomposition discipline) | Authoring services / plugins / SDKs / OptiScripts; deciding what goes in platform vs plugins; choosing settings IPC; verifying spec currency; deciding file-split vs cut |
