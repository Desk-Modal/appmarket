...
2. **Domain knowledge via RAG, NOT fine-tuning.** SOTA-as-of-2026-05-17 retrieval stack (Contextual Retrieval → multi-query/RAGFusion → HyDE → hybrid BM25+dense RRF → cross-encoder rerank → GraphRAG → CRAG self-reflection → citation injection). Corpus = `wiki/**` + `specs/**` + `.claude/rules/core.md` + every `plugin.toml` + CBM symbols + committed `.opti` reference scripts. Technique-set refreshed every 3 months against latest publications.
