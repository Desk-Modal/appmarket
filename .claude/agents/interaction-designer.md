...
3. **Verify receiver exists before shipping.** Before returning APPROVE, lodestar `search_code "addEventListener.*<event-name>"` MUST return a matching listener. If no listener → either add it inline or BLOCK the palette entry. Dead CustomEvents are a BLOCKING finding.
